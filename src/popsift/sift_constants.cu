...
void init_constants(
  float sigma0, int levels, float threshold, float edge_limit, int max_extrema, int normalization_multiplier)
{
    cudaError_t err;
    ...
    err = cudaMemcpyToSymbol(d_consts, &h_consts, sizeof(ConstInfo), 0, cudaMemcpyHostToDevice);
