...
// Seems like the template argument HEIGHT is not used for anything
template<int HEIGHT>
__device__ static inline uint32_t extrema_count(unsigned int indicator, int* extrema_counter)
{
    uint32_t mask = popsift::ballot(indicator); // bitfield of warps with results

    int ct = __popc(mask); // horizontal reduce

    // if(blockIdx.x == 5 && blockIdx.y == 8 && blockIdx.z == 0 && threadIdx.x == 0 && threadIdx.y == 0 && gridDim.x >
    // 35)
    // if(blockIdx.z == 0 && threadIdx.x == 0 && threadIdx.y == 0 && gridDim.x > 35)
    //     // printf("\t\tct = %d, block(%d, %d)\n", ct, blockIdx.x, blockIdx.y);

    if(ct > 1 && threadIdx.x == 0 && gridDim.x > 35) // only first octave
    {
        printf("ct = %d -- block(%d, %d)\n", ct, blockIdx.x, blockIdx.y);
    }

    // 4 warps per block 3 dimensions in z hence each block ran 3 times causing print to occur 4*3=12 times
    // if(blockIdx.x == 5 && blockIdx.y == 8 && threadIdx.x == 0 && gridDim.x > 35)
    // {
    //     printf("ct = %d -- block(%d, %d)\n", ct, blockIdx.x, blockIdx.y);
    // }
...
__device__ static inline void extremum_cmp(float val, float f, uint32_t& gt, uint32_t& lt, uint32_t mask)
{
    gt |= ((val > f) ? mask : 0);
    lt |= ((val < f) ? mask : 0);
}
...
__device__ static inline bool is_extremum(cudaTextureObject_t obj, int x, int y, int z)
{
    uint32_t gt = 0;
    uint32_t lt = 0;
...
class ModeFunctions
{
  public:
     ...
     */
    inline __device__ int refine(float3& d, int3& n, int width, int height, int maxlevel, bool last_it);
};
...
{
  public:
    inline __device__ int refine(float3& d, int3& n, int width, int height, int maxlevel, bool last_it) const
    {
        if(last_it)
            return 0;
...
__device__ inline static bool first_contrast_ok(const float val) { return (fabsf(val) >= 1.6f * d_consts.threshold); }
 ...
 *          false : they do not
 */
__device__ inline static bool verify(float xn, float yn, float sn, int width, int height, int maxlevel)
{
                                               ...
                                               InitialExtremum& ec)
{
    ...
    // if(blockIdx.x == 2 && blockIdx.y == 5 && blockIdx.z == 0)
    const float val = readTex(dog, x, y, level);
    // if(block_x == 32 && block_y == 12 && block_z == 0 && width == 1280)
    // {
    //     printf("x=%d - y=%d - z=%d --- val=%f from readTex(dog, %d, %d, %d)\n", x, y, level, val, x, y, level);
    //     if(x == 33)
    //         printf("val=%f from readTex(dog, %d, %d, %d)\n", readTex(dog, 16, 38, 1), 16, 38, 1);
    // }
...
__global__ void find_extrema_in_dog(cudaTextureObject_t dog,
                                    int octave,
                                    int width,
                                    int height,
                                    ...
                                    const int grid_width)
{
    ...
    bool indicator = find_extrema_in_dog_sub<sift_mode>(
      dog, octave, width, height, maxlevel, w_grid_divider, h_grid_divider, grid_width, ec);
    ...
    {
        const int block_x = blockIdx.x * 32 + threadIdx.x;
        const int block_y = blockIdx.y * blockDim.y + threadIdx.y;
        const int block_z = blockIdx.z; // no thread idx for z

        // if(block_x == 1193 && block_y == 419 && block_z == 1)
        // if(block_x == 640 && block_y == 221 && block_z == 0)
        if(block_x == 17 && block_y == 750 && block_z == 0)
        {
            printf("Initial extremum at pos(%d, %d %d): xpos=%f ypos=%f -- lpos=%d "
    ...
    if(indicator && write_index < d_consts.max_extrema)
    {
        ...
        if(write_index == 0)
            printf("ec --> xpos = %f ypos = %f", ec.xpos, ec.ypos);

        // index for indirect access to d_extrema, to enable
        // access after filtering some initial extrema
        d_ext_off[write_index] = write_index; // don't understand how this is usefull
                                              // just an array where each pos in array says the pos in the array
                                              // just moved based on octave base octave * 100 000??
    }
    ...
    if(width == 1280 && blockIdx.x == 0 && blockIdx.y == 0)
    {
        // printf("DOING THE ATOMIC ADD THINGY THREADIDX.x and y is 0, width=%d\n", width);
        // printf("number of blocks=%d", number_of_blocks);
    }
    if(threadIdx.x == 0 && threadIdx.y == 0)
    {
        int ct = atomicAdd(d_number_of_blocks, 1); // increments and returns old value to ct
        if(ct >= number_of_blocks - 1) // Runs alot on my computer final ct is 25560 for number_of_blocks = 8520
        {
            // You are the last block so everyone have updated dct.ext_ct[octave] and the value there is now complete
            // and we konw how many extremum there is in this octave
            int num_ext = atomicMin(&dct.ext_ct[octave], d_consts.max_extrema);
            // the purpose of storig this value I do not see?? as dct.ext_ct[octave] will be unchnaged if it's less than
            // d_consts.max_extrema and set to d_const.max_extrema if it is larger
...
__global__ void printExtremaCount(int octave, int* d_number_of_blocks)
{
    printf("\n\tdct.ext_ct[%d] = %d  --  _d_extrema_num_blocks[%d]=%d\n",
           octave,
           dct.ext_ct[octave],
           octave,
           *d_number_of_blocks);
...
__host__ void Pyramid::find_extrema(const Config& conf)
{
    ...
    for(int octave = 0; octave < _num_octaves; octave++)
    {
        ...
        switch(conf.getSiftMode())
        {
            case Config::RefineInLevel:
                printf("RefineInLevel type VLfeat");
        ...
        // cudaDeviceSynchronize(); // sync and then we print the final extrema count

        // if(cols == 1280 && octave == 0)
        // printExtremaCount<<<1, 1>>>(octave, extrema_num_blocks);

        // Event seems to only be recorded but not used for anything as far as I can see in this branch
