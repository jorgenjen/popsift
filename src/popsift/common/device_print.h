...
__global__ static void printSurface(
  cudaSurfaceObject_t obj, const char* identifier, int start_x, int end_x, int start_y, int end_y)
{
    ...
    for(int y = start_y; y < end_y; ++y)
    {
        ...
        {
            surf2DLayeredread(&val, obj, x * 4, y, 0, cudaBoundaryModeZero);
            printf("%10.6f ", val);
        }
        printf("\n");
    }
...
void print_region(T obj, const char* identifier, int start_x, int end_x, int start_y, int end_y)
{
    ...
    if(is_tex)
    {
        printTexture<<<1, 1>>>(obj, dev_msg, start_x, end_x, start_y, end_y);
    }
    else
    {
        printSurface<<<1, 1>>>(obj, dev_msg, start_x, end_x, start_y, end_y);
    }
    cudaDeviceSynchronize();
