...
struct PlaneBase
{
    __host__ void* allocDev2D(size_t& pitch, int w, int h, int elemSize);

    __host__ void freeDev2D(void* data);

    __host__ void* allocHost2D(int w, int h, int elemSize, PlaneMapMode m);

    __host__ void freeHost2D(void* data, PlaneMapMode m);

    __host__ void memcpyToDevice(
      void* dst, int dst_pitch, void* src, int src_pitch, short cols, short rows, int elemSize);

    __host__ void memcpyToDevice(
      void* dst, int dst_pitch, void* src, int src_pitch, short cols, short rows, int elemSize, cudaStream_t stream);

    __host__ void memcpyToHost(
      void* dst, int dst_pitch, void* src, int src_pitch, short cols, short rows, int elemSize);
    ...
    __host__ void waitAndCheck(cudaStream_t stream) const;
#else  // not PLANE2D_CUDA_OP_DEBUG
    __host__ inline void waitAndCheck(cudaStream_t stream) const {}
...
template<typename T>
struct PlaneT : public PlaneBase
{
    typedef T elem_type;
    ...
    __host__ __device__ PlaneT()
      : data(0)
    {}
    __host__ __device__ explicit PlaneT(T* d)
      : data(d)
    {}
...
template<typename T>
struct PitchPlane2D : public PlaneT<T>
{
    __host__ __device__ PitchPlane2D()
      : _pitchInBytes(0)
    {}

    __host__ __device__ PitchPlane2D(T* d, int s)
      : PlaneT<T>(d)
      , _pitchInBytes(s)
    {}
    ...
    __host__ inline void memcpyToDevice(PitchPlane2D<T>& devPlane, short cols, short rows);
    __host__ inline void memcpyToDevice(PitchPlane2D<T>& devPlane, short cols, short rows, cudaStream_t stream);

    /** cuda memcpy from parameter (plane allocated on host) to
     *  this (plane allocated on device) */
    __host__ inline void memcpyFromHost(PitchPlane2D<T>& hostPlane, short cols, short rows);
    __host__ inline void memcpyFromHost(PitchPlane2D<T>& hostPlane, short cols, short rows, cudaStream_t stream);

    /** cuda memcpy from parameter (plane allocated on device) to
     *  this (plane allocated on host) */
    __host__ inline void memcpyFromDevice(PitchPlane2D<T>& devPlane, short cols, short rows);
    __host__ inline void memcpyFromDevice(PitchPlane2D<T>& devPlane, short cols, short rows, cudaStream_t stream);

    /** cuda memcpy from this (plane allocated on device) to
     *  parameter (plane allocated on host) */
    __host__ inline void memcpyToHost(PitchPlane2D<T>& hostPlane, short cols, short rows);
    __host__ inline void memcpyToHost(PitchPlane2D<T>& hostPlane, short cols, short rows, cudaStream_t stream);

    __host__ __device__ inline const T* ptr(int y) const
    {
        return (const T*)((const char*)this->data + y * _pitchInBytes);
    }
    __host__ __device__ inline T* ptr(int y) { return (T*)((char*)this->data + y * _pitchInBytes); }
    ...
    __host__ inline void allocDev(int w, int h)
    {
        ...
        std::cout << "PITCH: -- " << pitch << " widht: " << w << " Height: " << h << " elemSize: " << this->elemSize()
                  << std::endl;
    }
    ...
    __host__ inline void freeHost(PlaneMapMode mode) { PlaneBase::freeHost2D(this->data, mode); }
    __host__ __device__ inline size_t getPitchInBytes() const { return _pitchInBytes; }
...
template<typename T>
__host__ inline void PitchPlane2D<T>::memcpyToDevice(PitchPlane2D<T>& devPlane, short cols, short rows)
{
    PlaneBase::memcpyToDevice(
      devPlane.data, devPlane._pitchInBytes, this->data, this->_pitchInBytes, cols, rows, sizeof(T));
}

template<typename T>
__host__ inline void PitchPlane2D<T>::memcpyToDevice(PitchPlane2D<T>& devPlane,
                                                     short cols,
                                                     short rows,
                                                     cudaStream_t stream)
{
    PlaneBase::memcpyToDevice(
      devPlane.data, devPlane._pitchInBytes, this->data, this->_pitchInBytes, cols, rows, sizeof(T), stream);
}

template<typename T>
__host__ inline void PitchPlane2D<T>::memcpyFromHost(PitchPlane2D<T>& hostPlane, short cols, short rows)
{
    hostPlane.memcpyToDevice(*this, cols, rows);
}

template<typename T>
__host__ inline void PitchPlane2D<T>::memcpyFromHost(PitchPlane2D<T>& hostPlane,
                                                     short cols,
                                                     short rows,
                                                     cudaStream_t stream)
{
...
template<typename T>
__host__ inline void PitchPlane2D<T>::memcpyFromDevice(PitchPlane2D<T>& devPlane, short cols, short rows)
{
    PlaneBase::memcpyToHost(
      this->data, this->_pitchInBytes, devPlane.data, devPlane._pitchInBytes, cols, rows, sizeof(T));
}

template<typename T>
__host__ inline void PitchPlane2D<T>::memcpyFromDevice(PitchPlane2D<T>& devPlane,
                                                       short cols,
                                                       short rows,
                                                       cudaStream_t stream)
{
    PlaneBase::memcpyToHost(
      this->data, this->_pitchInBytes, devPlane.data, devPlane._pitchInBytes, cols, rows, sizeof(T), stream);
}

template<typename T>
__host__ inline void PitchPlane2D<T>::memcpyToHost(PitchPlane2D<T>& hostPlane, short cols, short rows)
{
    hostPlane.memcpyFromDevice(*this, cols, rows);
}

template<typename T>
__host__ inline void PitchPlane2D<T>::memcpyToHost(PitchPlane2D<T>& hostPlane,
                                                   short cols,
                                                   short rows,
                                                   cudaStream_t stream)
{
 ...
 *************************************************************/
template<typename T>
class Plane2D : public PitchPlane2D<T>
{
    short _cols;
    short _rows;

  public:
    __host__ __device__ Plane2D()
      : _cols(0)
      , _rows(0)
    {}

    __host__ __device__ Plane2D(int w, int h, T* d, int s)
      : PitchPlane2D<T>(d, s)
      , _cols(w)
      , _rows(h)
    {}

    __host__ __device__ Plane2D(int w, int h, const PitchPlane2D<T>& plane)
    ...
    template<typename U>
    __host__ __device__ explicit Plane2D(const Plane2D<U>& orig)
    ...
    __host__ __device__ inline short getCols() const { return _cols; }
    __host__ __device__ inline short getWidth() const { return _cols; }
    __host__ __device__ inline short getRows() const { return _rows; }
    __host__ __device__ inline short getHeight() const { return _rows; }
    __host__ __device__ inline size_t getByteSize() const { return this->_pitchInBytes * _rows; }
...
template<typename T>
__host__ void Plane2D<T>::resetDimensionsHost(int w, int h)
{
    this->_cols = w;
    this->_rows = h;
...
template<typename T>
__host__ void Plane2D<T>::resetDimensionsDev(int w, int h)
{
    // validate pitch
    ...
    this->_cols = w;
    this->_rows = h;
    // on the device side, memory is NOT contiguous (CUDA may add padding) => pitch can not be changed without
    // reallocation
}

template<typename T>
__host__ inline void Plane2D<T>::memcpyToDevice(Plane2D<T>& devPlane)
{
...
template<typename T>
__host__ inline void Plane2D<T>::memcpyToDevice(PitchPlane2D<T>& devPlane)
{
    PitchPlane2D<T>::memcpyToDevice(devPlane, this->_cols, this->_rows);
}

template<typename T>
__host__ inline void Plane2D<T>::memcpyToDevice(Plane2D<T>& devPlane, cudaStream_t stream)
{
    ...
    {
        std::stringstream ss;
        ss << "Error: source rows (" << this->_rows << ") and dest rows (" << devPlane._rows << ") must be identical";
        POP_FATAL(ss.str());
    }
...
template<typename T>
__host__ inline void Plane2D<T>::memcpyToDevice(PitchPlane2D<T>& devPlane, cudaStream_t stream)
{
    PitchPlane2D<T>::memcpyToDevice(devPlane, this->_cols, this->_rows, stream);
}

template<typename T>
__host__ inline void Plane2D<T>::memcpyFromHost(Plane2D<T>& hostPlane)
{
    hostPlane.memcpyToDevice(*this);
}

template<typename T>
__host__ inline void Plane2D<T>::memcpyFromHost(PitchPlane2D<T>& hostPlane)
{
    hostPlane.memcpyToDevice(*this, this->_cols, this->_rows);
}

template<typename T>
__host__ inline void Plane2D<T>::memcpyFromHost(Plane2D<T>& hostPlane, cudaStream_t stream)
{
    hostPlane.memcpyToDevice(*this, stream);
}

template<typename T>
__host__ inline void Plane2D<T>::memcpyFromHost(PitchPlane2D<T>& hostPlane, cudaStream_t stream)
{
    hostPlane.memcpyToDevice(*this, this->_cols, this->_rows, stream);
}

template<typename T>
__host__ inline void Plane2D<T>::memcpyFromDevice(Plane2D<T>& devPlane)
{
    assert(devPlane._cols == this->_cols);
    assert(devPlane._rows == this->_rows);
...
template<typename T>
__host__ inline void Plane2D<T>::memcpyFromDevice(PitchPlane2D<T>& devPlane)
{
    PitchPlane2D<T>::memcpyFromDevice(devPlane, this->_cols, this->_rows);
}

template<typename T>
__host__ inline void Plane2D<T>::memcpyFromDevice(Plane2D<T>& devPlane, cudaStream_t stream)
{
    assert(devPlane._cols == this->_cols);
    assert(devPlane._rows == this->_rows);
...
template<typename T>
__host__ inline void Plane2D<T>::memcpyFromDevice(PitchPlane2D<T>& devPlane, cudaStream_t stream)
{
    PitchPlane2D<T>::memcpyFromDevice(devPlane, this->_cols, this->_rows, stream);
}

template<typename T>
__host__ inline void Plane2D<T>::memcpyToHost(Plane2D<T>& hostPlane)
{
    hostPlane.memcpyFromDevice(*this);
}

template<typename T>
__host__ inline void Plane2D<T>::memcpyToHost(PitchPlane2D<T>& hostPlane)
{
    hostPlane.memcpyFromDevice(*this, this->_cols, this->_rows);
}

template<typename T>
__host__ inline void Plane2D<T>::memcpyToHost(Plane2D<T>& hostPlane, cudaStream_t stream)
{
    hostPlane.memcpyFromDevice(*this, stream);
}

template<typename T>
__host__ inline void Plane2D<T>::memcpyToHost(PitchPlane2D<T>& hostPlane, cudaStream_t stream)
{
