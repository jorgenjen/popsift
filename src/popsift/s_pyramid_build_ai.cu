...
__global__ static void horiz(cudaTextureObject_t src_linear_tex, cudaSurfaceObject_t dst_data, int dst_level)
{
    ...
    for(int offset = 1; offset <= span; offset += 2)
    {
        ...
        const float val =
          readTex(src_linear_tex, idx - off, idy, src_level) + readTex(src_linear_tex, idx + off, idy, src_level);
...
__global__ static void vert(cudaTextureObject_t src_linear_tex, cudaSurfaceObject_t dst_data, int dst_level)
{
    ...
    for(int offset = 1; offset <= span; offset += 2)
    {
        ...
        const float off = offset + (1.0f - u);
        const float val =
          readTex(src_linear_tex, idx, idy - off, dst_level) + readTex(src_linear_tex, idx, idy + off, dst_level);
...
__host__ void Pyramid::horiz_from_prev_level_pairs(int octave, int level, cudaStream_t stream)
{
    ...
    absoluteSourceInterpolated::horiz<<<grid, block, 0, stream>>>(
      oct_obj.getDataTexLinear().tex, oct_obj.getIntermediateSurface(), level);
    POP_SYNC_CHK;
}

__host__ void Pyramid::vert_from_interm_pairs(int octave, int level, cudaStream_t stream)
{
    Octave& oct_obj = _octaves[octave];
    ...
    absoluteSourceInterpolated::vert<<<grid, block, 0, stream>>>(
      oct_obj.getIntermDataTexLinear().tex, oct_obj.getDataSurface(), level);
    POP_SYNC_CHK;
}
