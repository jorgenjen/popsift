...
__global__ void get_by_2_pick_every_second(cudaTextureObject_t src_data,
                                           const int src_w,
                                           const int src_h,
                                           const int src_level,
...
__host__ inline void Pyramid::downscale_from_prev_octave(int octave, cudaStream_t stream)
{
...
__host__ inline void Pyramid::horiz_from_prev_level(int octave,
                                                    int level,
                                                    cudaStream_t stream,
                                                    GaussTableChoice useInterpolatedGauss)
{
    switch(useInterpolatedGauss)
    {
        ...
        case NotInterpolated_FromPrevious: horiz_from_prev_level_basic(octave, level, stream); break;
        default: POP_FATAL("Missing case in horizontal Gauss filter from previous level"); break;
    }
}

__host__ inline void Pyramid::vert_from_interm(int octave,
                                               int level,
                                               cudaStream_t stream,
                                               GaussTableChoice useInterpolatedGauss)
{
    ...
    printf("Use interpolated gauss = %d\n", useInterpolatedGauss);

    switch(useInterpolatedGauss)
    {
        case Interpolated_FromPrevious: vert_from_interm_pairs(octave, level, stream); break;
        case NotInterpolated_FromPrevious: vert_from_interm_basic(octave, level, stream); break;
        default: {
...
__host__ inline void Pyramid::dogs_from_blurred(int octave, int max_level, cudaStream_t stream)
{
    Octave& oct_obj = _octaves[octave];
...
__global__ static void printDoG(cudaSurfaceObject_t dog, int width, int height)
{
    printf("\nPRINT ME CUDA DOG's from y(%d -> %d) x(%d -> %d)\n", height - 8, height, width - 8, width);
    float val = {};
    for(int y = height - 8; y < height; ++y)
    {
        for(int x = width - 8; x < width; ++x)
        {
            ...
            surf2DLayeredread(&val, dog, x * 4, y, 0, cudaBoundaryModeZero);
            printf("%10.6f ", val);
        }
        printf("\n");
    }
 ...
 * V11: host side
 *************************************************************/
__host__ void Pyramid::build_pyramid(const Config& conf, ImageBase* base)
{
    ...
    cout << "Octaves: " << _num_octaves << endl;
    cout << "Levels: " << _levels << endl;
    cout << "Gauss choice: " << conf.getGaussMode() << endl;

    // Interpolated from previous seems to be dafault
