...
void Octave::alloc_data_planes()
{
    ...
    err = cudaMalloc3DArray(&_data, &_data_desc, _data_ext, cudaArrayLayered | cudaArraySurfaceLoadStore);
...
void Octave::alloc_interm_array()
{
    ...
    // Bits allocated for each channel. only allocated from one float for x
    _intm_desc.x = 32;
    _intm_desc.y = 0;
    _intm_desc.z = 0;
    _intm_desc.w = 0;
    ...
    _intm_ext.height = _h;
    printf("Levelsssss: %d", _levels);
    _intm_ext.depth = _levels;

    err = cudaMalloc3DArray(&_intm, &_intm_desc, _intm_ext, cudaArrayLayered | cudaArraySurfaceLoadStore);
...
void Octave::alloc_dog_array()
{
    ...
    err = cudaMalloc3DArray(&_dog_3d, &_dog_3d_desc, _dog_3d_ext, cudaArrayLayered | cudaArraySurfaceLoadStore);
...
void Octave::alloc_streams() { _stream = popsift::cuda::stream_create(__FILE__, __LINE__); }

void Octave::free_streams() { popsift::cuda::stream_destroy(_stream, __FILE__, __LINE__); }
