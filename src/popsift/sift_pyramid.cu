...
__global__ void py_print_corner_float(float* img, uint32_t pitch, uint32_t height, uint32_t level)
{
    const int xbase = 0;
...
__global__ void py_print_corner_float_transposed(float* img, uint32_t pitch, uint32_t height, uint32_t level)
{
    const int xbase = 0;
    const int ybase = level * height + 0;
    for(int i = 0; i < 10; i++)
  ...
  , _initial_blur(config.getInitialBlur())
{
    ...
    int sz = _num_octaves * h_consts.max_extrema; // _num_octaves
    printf("\n\t\th_consts.max_extrema =%d -- _num_octaves = %d sizeof(InitialExtremum) = %lu, sizeof(DevBuffer) = %lu",
           h_consts.max_extrema,
           _num_octaves,
           sizeof(InitialExtremum),
           sizeof(DevBuffers));
 ...
 * GPUs are compatible.
 */
__global__ void prep_features(Descriptor* descriptor_base, int up_fac)
{
...
void Pyramid::writeDescriptor(
  const Config& conf, ostream& ostr, FeaturesHost* features, bool really, bool with_orientation)
{
    ...
    for(int ext_idx = 0; ext_idx < hct.ext_total; ext_idx++)
    {
        ...
        for(int ori = 0; ori < ext.num_ori; ori++)
        {
            ...
            if(with_orientation)
                ostr << setprecision(5) << xpos << " " << ypos << " " << sigma << " " << dom_ori << " ";
            else
                ostr << setprecision(5) << xpos << " " << ypos << " " << 1.0f / (sigma * sigma) << " 0 "
                     << 1.0f / (sigma * sigma) << " ";
